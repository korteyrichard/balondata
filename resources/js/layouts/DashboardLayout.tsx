...
export default function DashboardLayout({ user, header, children }: DashboardLayoutProps) {
    ...
    const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => (
        <div className="flex flex-col flex-grow bg-sidebar pt-5 pb-4 overflow-y-auto h-full">
            <div className="flex items-center flex-shrink-0 px-4">
                <Link href="/">
                    <div className="text-sidebar-foreground text-lg font-bold flex flex-row gap-4 items-center justify-between">
                        <img src='/balondata.jpeg' alt="BalonData" className={`${isMobile ? 'w-200' : 'w-100'} h-30 mb-4 mx-auto rounded-3xl`} />
                    </div>
                </Link>
            </div>
            <div className="px-4 mb-4">
                <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 rounded-full bg-sidebar-accent flex items-center justify-center">
                        <span className="text-lg font-bold text-sidebar-accent-foreground">{user.name.charAt(0)}</span>
                    </div>
                    <div className="text-sidebar-foreground">
                        <h3 className="text-sm font-bold">{user.name}</h3>
                        <p className="text-xs text-sidebar-foreground/80">{user.role.toUpperCase()}</p>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            <div className="lg:pl-64 flex flex-col flex-1">
                {/* Navbar */}
                <div className="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-sidebar shadow-sm lg:shadow-none">
                    ...
                    <div className="flex-1 flex justify-between px-4">
                        <div className="flex-1 flex items-center">
                            {header && (
                                <h2 className="font-semibold text-xl text-sidebar-foreground leading-tight">
                                    {header}
                                </h2>
                            )}
                        </div>
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" className="relative h-8 w-8 rounded-full">
                                        <Icon name="User" className="h-6 w-6 text-sidebar-foreground" />
                                    </Button>
                                </DropdownMenuTrigger>
